...
set "R4OS_R4CC_SETTING="
set "R4OS_R4PACK_SETTING="
...
for /f "usebackq tokens=1,* delims==" %%A in ("%R4OS_SETTINGS%") do (
    ...
    if /i "%%A"=="R4CC_ROOT" set "R4OS_R4CC_SETTING=%%B"
    if /i "%%A"=="R4PACK_ROOT" set "R4OS_R4PACK_SETTING=%%B"
    if /i "%%A"=="SDK_ROOT" set "R4OS_SDK_SETTING=%%B"
    if /i "%%A"=="ZIG_ROOT" set "R4OS_ZIG_SETTING=%%B"
)

for %%K in (ARTIFACTS CONTRACT DEVKIT R4CC R4PACK SDK ZIG) do if not defined R4OS_%%K_SETTING (
...
for %%I in ("%R4OS_R4CC_SETTING%") do set "R4OS_R4CC_ROOT=%%~fI"
for %%I in ("%R4OS_R4PACK_SETTING%") do set "R4OS_R4PACK_ROOT=%%~fI"
...
if not exist "%R4OS_R4CC_ROOT%\build.zig.zon" (
    echo ERROR: R4CC repository not found: "%R4OS_R4CC_ROOT%"
    exit /b 1
)
if not exist "%R4OS_R4PACK_ROOT%\build.zig.zon" (
    echo ERROR: R4Pack repository not found: "%R4OS_R4PACK_ROOT%"
    exit /b 1
)
...
set "R4OS_FORKS=%R4OS_FORKS% --fork="%R4OS_R4CC_ROOT%" --fork="%R4OS_R4PACK_ROOT%""
