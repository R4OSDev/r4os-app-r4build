...
while IFS='=' read -r key value; do
    case "$key" in
        ...
        R4CC_ROOT) r4cc_setting=$value ;;
        R4PACK_ROOT) r4pack_setting=$value ;;
...
require_setting R4CC_ROOT "$r4cc_setting"
require_setting R4PACK_ROOT "$r4pack_setting"
...
r4cc_root=$(resolve_path "$module_root" "$r4cc_setting")
r4pack_root=$(resolve_path "$module_root" "$r4pack_setting")
...
if [ ! -f "$r4cc_root/build.zig.zon" ]; then
    echo "ERROR: R4CC repository not found: $r4cc_root" >&2
    exit 1
fi
if [ ! -f "$r4pack_root/build.zig.zon" ]; then
    echo "ERROR: R4Pack repository not found: $r4pack_root" >&2
    exit 1
fi
...
if [ -n "$libraries_root" ]; then
    exec "$zig_exe" build --prefix "$artifacts_root" "--fork=$sdk_root" "--fork=$contract_root" "--fork=$libraries_root" "--fork=$r4cc_root" "--fork=$r4pack_root" "$@"
fi
exec "$zig_exe" build --prefix "$artifacts_root" "--fork=$sdk_root" "--fork=$contract_root" "--fork=$r4cc_root" "--fork=$r4pack_root" "$@"
